((Utils) => {
    const App = {
      htmlElements: {
        ...
        response: document.querySelector('.pokelimpieza'),
      },
      init: () => { 
        ...
        pokemonFinderFormOnSubmit: async (e) => {
          ...
          try {
            const response = await Utils.getPokemon({   
              query,
              searchType,
            }); // Se obtiene la informacion del pokemon csm
      ...
      templates: {
        ...
        showNewInput(){
          const valido = confirm("¿Deseas Poke-limpiar?")
          if(valido==true)
